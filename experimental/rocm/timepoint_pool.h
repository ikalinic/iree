...
#ifndef EXPERIMENTAL_ROCM_TIMEPOINT_POOL_H_
#define EXPERIMENTAL_ROCM_TIMEPOINT_POOL_H_
...
typedef struct iree_hal_rocm_timepoint_pool_t iree_hal_rocm_timepoint_pool_t;

// An enum to identify the timepoint kind in iree_hal_rocm_timepoint_t objects.
typedef enum iree_hal_rocm_timepoint_kind_e {
  // None; for uninitialized timepoint objects.
  IREE_HAL_ROCM_TIMEPOINT_KIND_NONE = 0,
  // A timepoint waited by the host.
  IREE_HAL_ROCM_TIMEPOINT_KIND_HOST_WAIT,
  // A timepoint signaled by the device.
  IREE_HAL_ROCM_TIMEPOINT_KIND_DEVICE_SIGNAL,
  // A timepoint waited by the device.
  IREE_HAL_ROCM_TIMEPOINT_KIND_DEVICE_WAIT,
} iree_hal_rocm_timepoint_kind_t;

// An object that wraps a host iree_event_t or device iree_hal_rocm_event_t to
// represent wait/signal of a timepoint on a timeline.
//
// iree_hal_rocm_timepoint_t objects cannot be directly created; it should be
...
typedef struct iree_hal_rocm_timepoint_t {
  ...
  iree_hal_rocm_timepoint_pool_t* pool;

  iree_hal_rocm_timepoint_kind_t kind;
  union {
    iree_event_t host_wait;
    iree_hal_rocm_event_t* device_signal;
    // The device event to wait. NULL means no device event available to wait
    // for this timepoint at the moment.
    iree_hal_rocm_event_t* device_wait;
  } timepoint;
} iree_hal_rocm_timepoint_t;
...
// A simple pool of iree_hal_rocm_timepoint_t objects to recycle.
//
...
#endif  // EXPERIMENTAL_ROCM_TIMEPOINT_POOL_H_
