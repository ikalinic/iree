...
static void iree_hal_rocm_device_destroy(iree_hal_device_t* base_device) {
  ...
  const iree_hal_rocm_dynamic_symbols_t* syms = device->context_wrapper.syms;
...
static iree_status_t iree_hal_rocm_device_create_internal(
    ...
    hipDevice_t rocm_device, hipStream_t dispatch_stream,
    hipStream_t callback_stream, hipCtx_t context,
  ...
  iree_status_t status = iree_hal_rocm_pending_queue_actions_create(
      &device->context_wrapper, &device->block_pool, host_allocator,
...
iree_status_t iree_hal_rocm_device_create(iree_hal_driver_t* driver,
  ...
  if (iree_status_is_ok(status)) {
    status = iree_hal_rocm_device_create_internal(
        driver, identifier, device, dispatch_stream, callback_stream, context,
        syms, host_event_pool, device_event_pool, timepoint_pool,
...
static iree_status_t iree_hal_rocm_device_create_semaphore(
  ...
  iree_hal_rocm_device_t* device = iree_hal_rocm_device_cast(base_device);
  return iree_hal_rocm_semaphore_create(
