...
typedef struct iree_hal_rocm_semaphore_t {
  ...
  // // The symbols used to issue ROCM API calls.
...
static iree_status_t iree_hal_rocm_semaphore_signal(
  ...
  iree_status_t status = iree_hal_rocm_pending_queue_actions_issue(
...
static iree_status_t iree_hal_rocm_semaphore_timepoint_host_wait_callback(
    void* user_data, iree_hal_semaphore_t* semaphore, uint64_t value,
    iree_status_code_t status_code) {
  iree_hal_rocm_timepoint_t* timepoint = (iree_hal_rocm_timepoint_t*)user_data;
...
static iree_status_t iree_hal_rocm_semaphore_acquire_timepoint_host_wait(
    iree_hal_rocm_semaphore_t* semaphore, uint64_t min_value,
    iree_timeout_t timeout, iree_hal_rocm_timepoint_t** out_timepoint) {
  IREE_TRACE_ZONE_BEGIN(z0);

  IREE_RETURN_AND_END_ZONE_IF_ERROR(
      z0, iree_hal_rocm_timepoint_pool_acquire_host_wait(
  ...
  iree_hal_semaphore_acquire_timepoint(
      ...
      (iree_hal_semaphore_callback_t){
          .fn = iree_hal_rocm_semaphore_timepoint_host_wait_callback,
...
static iree_status_t iree_hal_rocm_semaphore_wait(
  ...
  iree_status_t status = iree_hal_rocm_semaphore_acquire_timepoint_host_wait(
  ...
  iree_hal_rocm_timepoint_pool_release(semaphore->timepoint_pool, 1,
                                       &timepoint);
  IREE_TRACE_ZONE_END(z0);
  return status;
}
...
// advances past the given |value|.
//
// Note that this callback is invoked by the a host thread after the ROCM host
// function callback function is triggered in the ROCM driver.
static iree_status_t iree_hal_rocm_semaphore_timepoint_device_signal_callback(
    void* user_data, iree_hal_semaphore_t* semaphore, uint64_t value,
    iree_status_code_t status_code) {
  iree_hal_rocm_timepoint_t* timepoint = (iree_hal_rocm_timepoint_t*)user_data;
  // Just release the timepoint back to the pool. This will decrease the
  // reference count of the underlying ROCM event internally.
  iree_hal_rocm_timepoint_pool_release(timepoint->pool, 1, &timepoint);
  return iree_ok_status();
}
...
iree_status_t iree_hal_rocm_event_semaphore_acquire_timepoint_device_signal(
  ...
  iree_hal_rocm_semaphore_t* semaphore =
      iree_hal_rocm_semaphore_cast(base_semaphore);
  iree_hal_rocm_timepoint_t* signal_timepoint = NULL;
  IREE_TRACE_ZONE_BEGIN(z0);

  IREE_RETURN_AND_END_ZONE_IF_ERROR(
      z0, iree_hal_rocm_timepoint_pool_acquire_device_signal(
  ...
  iree_hal_semaphore_acquire_timepoint(
      ...
      (iree_hal_semaphore_callback_t){
          .fn = iree_hal_rocm_semaphore_timepoint_device_signal_callback,
  ...
  for (iree_hal_semaphore_timepoint_t* tp = semaphore->base.timepoint_list.head;
       tp != NULL; tp = tp->next) {
    iree_hal_rocm_timepoint_t* wait_timepoint = (iree_hal_rocm_timepoint_t*)tp;
    if (wait_timepoint->kind == IREE_HAL_ROCM_TIMEPOINT_KIND_DEVICE_WAIT &&
  ...
  *out_event = iree_hal_rocm_event_handle(event);
  IREE_TRACE_ZONE_END(z0);
  return iree_ok_status();
}
...
//
// Note that this callback is invoked by the a host thread.
static iree_status_t iree_hal_rocm_semaphore_timepoint_device_wait_callback(
    void* user_data, iree_hal_semaphore_t* semaphore, uint64_t value,
    iree_status_code_t status_code) {
  iree_hal_rocm_timepoint_t* timepoint = (iree_hal_rocm_timepoint_t*)user_data;
  // Just release the timepoint back to the pool. This will decrease the
...
iree_status_t iree_hal_rocm_event_semaphore_acquire_timepoint_device_wait(
  ...
  iree_hal_rocm_semaphore_t* semaphore =
      iree_hal_rocm_semaphore_cast(base_semaphore);
  iree_hal_rocm_timepoint_t* wait_timepoint = NULL;
  IREE_TRACE_ZONE_BEGIN(z0);

  IREE_RETURN_AND_END_ZONE_IF_ERROR(
      z0, iree_hal_rocm_timepoint_pool_acquire_device_wait(
  ...
  iree_hal_semaphore_acquire_timepoint(
      ...
      (iree_hal_semaphore_callback_t){
          .fn = iree_hal_rocm_semaphore_timepoint_device_wait_callback,
