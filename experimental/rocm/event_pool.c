...
static inline void iree_hal_rocm_event_destroy(iree_hal_rocm_event_t* event) {
  ...
  iree_hal_rocm_dynamic_symbols_t* symbols = event->symbols;
...
void iree_hal_rocm_event_retain(iree_hal_rocm_event_t* event) {
...
struct iree_hal_rocm_event_pool_t {
  ...
  // Total number of hiprrently available event objects.
  iree_host_size_t available_count IREE_GUARDED_BY(event_mutex);
  // The list of available_count event objects.
  iree_hal_rocm_event_t* available_list[] IREE_GUARDED_BY(event_mutex);
};
...
iree_status_t iree_hal_rocm_event_pool_allocate(
  ...
  *out_event_pool = NULL;
  ...
  iree_hal_rocm_event_pool_t* event_pool = NULL;
  ...
  for (iree_host_size_t i = 0; i < available_capacity; ++i) {
    status = iree_hal_rocm_event_create(
...
static void iree_hal_rocm_event_pool_free(
  ...
  for (iree_host_size_t i = 0; i < event_pool->available_count; ++i) {
    iree_hal_rocm_event_t* event = event_pool->available_list[i];
    iree_atomic_ref_count_dec(&event->ref_count);  // -> 0
    iree_hal_rocm_event_destroy(event);
  }
...
iree_status_t iree_hal_rocm_event_pool_acquire(
    iree_hal_rocm_event_pool_t* event_pool, iree_host_size_t event_count,
    iree_hal_rocm_event_t** out_events) {
  ...
  // new iree_hal_rocm_event_t objects.
...
static void iree_hal_rocm_event_pool_release_event(
  ...
  if (to_pool_count > 0) {
    for (iree_host_size_t i = 0; i < to_pool_count; ++i) {
      ...
      iree_hal_rocm_event_retain(events[i]);  // -> 1
    }
  ...
  if (remaining_count > 0) {
    ...
    for (iree_host_size_t i = 0; i < remaining_count; ++i) {
      iree_hal_rocm_event_destroy(events[to_pool_count + i]);
    }
    IREE_TRACE_ZONE_END(z1);
  }
  IREE_TRACE_ZONE_END(z0);
