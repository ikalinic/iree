...
typedef enum iree_hal_rocm_queue_action_kind_e {
  IREE_HAL_ROCM_QUEUE_ACTION_TYPE_EXECUTION,
  // TODO: Add support for queue alloca and dealloca.
} iree_hal_rocm_queue_action_kind_t;
...
typedef struct iree_hal_rocm_queue_action_list_t {
  iree_hal_rocm_queue_action_t* head;
  iree_hal_rocm_queue_action_t* tail;
} iree_hal_rocm_queue_action_list_t;

// Returns true if the action list is empty.
static inline bool iree_hal_rocm_queue_action_list_is_empty(
    const iree_hal_rocm_queue_action_list_t* list) {
  return list->head == NULL;
}

// Pushes |action| on to the end of the given action |list|.
static void iree_hal_rocm_queue_action_list_push_back(
    iree_hal_rocm_queue_action_list_t* list,
...
static void iree_hal_rocm_queue_action_list_erase(
    iree_hal_rocm_queue_action_list_t* list,
    iree_hal_rocm_queue_action_t* action) {
  iree_hal_rocm_queue_action_t* next = action->next;
  iree_hal_rocm_queue_action_t* prev = action->prev;
  if (prev) {
    prev->next = next;
    action->prev = NULL;
  } else {
...
static void iree_hal_rocm_queue_action_list_take_all(
    iree_hal_rocm_queue_action_list_t* available_list,
    iree_hal_rocm_queue_action_list_t* ready_list) {
...
struct iree_hal_rocm_pending_queue_actions_t {
  ...
  // ROCM context used for making API calls
...
static const iree_hal_resource_vtable_t
    iree_hal_rocm_pending_queue_actions_vtable;
...
iree_status_t iree_hal_rocm_pending_queue_actions_create(
  ...
  iree_hal_rocm_pending_queue_actions_t* actions = NULL;
  IREE_RETURN_AND_END_ZONE_IF_ERROR(
      z0, iree_allocator_malloc(host_allocator, sizeof(*actions),
                                (void**)&actions));
  iree_hal_resource_initialize(&iree_hal_rocm_pending_queue_actions_vtable,
                               &actions->resource);
...
static iree_hal_rocm_pending_queue_actions_t*
iree_hal_rocm_pending_queue_actions_cast(iree_hal_resource_t* base_value) {
  return (iree_hal_rocm_pending_queue_actions_t*)base_value;
}

void iree_hal_rocm_pending_queue_actions_destroy(
...
static void iree_hal_rocm_free_semaphore_list(
    iree_allocator_t host_allocator,
...
iree_status_t iree_hal_rocm_pending_queue_actions_enqueue_execution(
  ...
  if (IREE_LIKELY(iree_status_is_ok(status))) {
    ...
    iree_hal_rocm_queue_action_list_push_back(&actions->action_list, action);
    iree_slim_mutex_unlock(&actions->action_mutex);
  } else {
    iree_hal_rocm_free_semaphore_list(actions->host_allocator,
                                      &action->wait_semaphore_list);
    iree_hal_rocm_free_semaphore_list(actions->host_allocator,
                                      &action->signal_semaphore_list);
...
static void iree_hal_rocm_pending_queue_actions_cleanup_execution(
    iree_hal_rocm_queue_action_t* action);

// Releases resources after action completion on the GPU and advances timeline
// and pending actions queue.
//
// This is the ROCM host function callback to rocmLaunchHostFunc, invoked by a
// ROCM driver thread.
static void iree_hal_rocm_execution_device_signal_host_callback(
    void* user_data) {
  iree_hal_rocm_queue_action_t* action =
      (iree_hal_rocm_queue_action_t*)user_data;
  iree_hal_rocm_pending_queue_actions_t* actions = action->owning_actions;
  ...
  // resources.
  iree_hal_rocm_pending_queue_actions_cleanup_execution(action);
  // Try to release more pending actions to the GPU now.
  IREE_IGNORE_ERROR(iree_hal_rocm_pending_queue_actions_issue(actions));
}
...
static iree_status_t iree_hal_rocm_pending_queue_actions_issue_execution(
  ...
  for (iree_host_size_t i = 0; i < action->payload.command_buffers.count; ++i) {
    ...
    ROCM_RETURN_AND_END_ZONE_IF_ERROR(
        z0, context->syms, hipGraphLaunch(exec, action->dispatch_hip_stream),
        "hipGraphLaunch");
  }
  ...
  for (iree_host_size_t i = 0; i < action->signal_semaphore_list.count; ++i) {
    ...
    IREE_RETURN_AND_END_ZONE_IF_ERROR(
        z0, iree_hal_rocm_event_semaphore_acquire_timepoint_device_signal(
    ...
    ROCM_RETURN_AND_END_ZONE_IF_ERROR(
        z0, context->syms, hipEventRecord(event, action->dispatch_hip_stream),
        "hipEventRecord");
...
iree_status_t iree_hal_rocm_pending_queue_actions_issue(
  ...
  iree_hal_rocm_queue_action_list_t pending_list = {NULL, NULL};
  iree_hal_rocm_queue_action_list_t ready_list = {NULL, NULL};

  iree_slim_mutex_lock(&actions->action_mutex);

  if (iree_hal_rocm_queue_action_list_is_empty(&actions->action_list)) {
    iree_slim_mutex_unlock(&actions->action_mutex);
    IREE_TRACE_ZONE_END(z0);
    return iree_ok_status();
  }
  ...
  for (iree_hal_rocm_queue_action_t* action = actions->action_list.head;
       action != NULL;) {
    ...
    if (action->is_pending) {
      iree_hal_rocm_queue_action_list_push_back(&pending_list, action);
    } else {
      iree_hal_rocm_queue_action_list_push_back(&ready_list, action);
    }
